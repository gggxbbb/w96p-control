...
import type { BatteryInfo, PowerStatus, MotorInfo, PowerConfigRegs, BleSnapshot } from '@gggxbbb/w96p-ble-sdk';
...
export const useDeviceStore = create<DeviceState>()(
  subscribeWithSelector((set) => ({
    ...
    setSnapshot: (snap) => set((state) => {
      ...
      else if (snap.battery !== undefined) next.battery = snap.battery as BatteryInfo | null;
      if (snap.powerStatus && state.powerStatus) next.powerStatus = { ...state.powerStatus, ...snap.powerStatus };
      else if (snap.powerStatus !== undefined) next.powerStatus = snap.powerStatus as PowerStatus | null;
      if (snap.motor && state.motor) next.motor = { ...state.motor, ...snap.motor };
      else if (snap.motor !== undefined) next.motor = snap.motor as MotorInfo | null;
      if (snap.powerConfig && state.powerConfig) next.powerConfig = { ...state.powerConfig, ...snap.powerConfig };
      else if (snap.powerConfig !== undefined) next.powerConfig = snap.powerConfig as PowerConfigRegs | null;
...
// Subscribe to device store changes for battery learning
